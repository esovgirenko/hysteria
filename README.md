...
Важно: нельзя честно гарантировать, что “ни одна DPI система не вычислит” туннель. Лучший режим — с доменом и валидным публичным TLS. Режим без домена нужен как быстрый временный старт по IP: он работает, но требует `allowInsecure` на клиенте и маскируется слабее.
...
- включит внутренний TLS-сертификат Caddy;
- запишет `insecure: true` в клиентские параметры.
