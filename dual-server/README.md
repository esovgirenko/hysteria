...
| Сервер 1 -> РФ-ресурсы | direct | Российские ресурсы открываются с IP Сервера 1 |
| Сервер 1 -> Сервер 2 | WireGuard | Быстрый стабильный туннель для зарубежных ресурсов |
| Сервер 2 -> интернет | NAT | Зарубежный выход в интернет |
...
flowchart LR
    ...
    XRAY -->|"geoip:ru / geosite:ru direct"| RUS["RU resources"]
    XRAY -->|"other traffic, policy route from 10.77.0.1"| WG1
...
- загружает `geoip.dat` и `geosite.dat`;
- настраивает `wg0`, но не меняет системный default route;
- добавляет policy routing только для источника `10.77.0.1`;
- настраивает Xray так, чтобы РФ-направления выходили через Сервер 1, а остальное через Сервер 2;
...
Проверить split-routing удобнее с клиента:

- российский сайт должен видеть IP Сервера 1;
- зарубежный сайт должен видеть IP Сервера 2.
